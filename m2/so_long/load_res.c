...
/*   Updated: 2026/02/02 20:00:05 by sohuikim         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
"./textures/floor.xpm", &ts->floor.img_w, &ts->floor.img_h);
...
int	create_bg_layer(t_mlx_vars *vars, t_img *frame_buffer, t_tileset *tileset, t_map *dst_size)
{
	...
	create_floor_layer(frame_buffer, &tileset->floor, dst_size);
}
...
void	create_wall_layer(t_img *dst, t_img *src, t_map *dst_size)
{
	while(dst_size->cnt_row)
	{
		while (dst_size->cnt_row)
	}
}
