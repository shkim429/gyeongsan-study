...
static bool	stop_philos(t_shared_data *data, bool *need_stop);
...
static bool	monitor_philos(t_table *table, bool *need_stop)
{
	...
	while (i < table->data.philo_num)
	{
		...
		if (state == PHILO_DEAD)
		{
			if (!stop_philos(&table->data, need_stop))
				return (false);
	...
	if (all_full)
		return (stop_philos(&table->data, need_stop));
	return (true);
}
